...
#' @export
translate_survey <- function(items_obj, chat = NULL, llm_model = NULL, api_key = NULL, sleep = 7) {
  ...
  EXAMPLES <- items_obj$example_translations
  ...
  if (!is.null(items_obj$batch_size)) {
    return(.batch_by_size(items_obj, bp, chat, sleep))
  }

  .batch_by_vars(items_obj, bp, EXAMPLES, chat, sleep)
}
...
.batch_by_size <- function(items_obj, bp, chat, sleep) {
...
#'
#' @export
.batch_by_vars <- function(items_obj, bp, EXAMPLES, chat, sleep) {
  ...
  for (batch_idx in 1:num_batches) {
    bp <- base_prompt(
      ...
      topic = batches[batch_idx, get(batch_vars[2])],
      domain = domain,
